...
# This script will initiate the transition to protocol version 6 (Alonzo fixes).
...
# 5 (which happens one or two epoch boundaries after invoking update-5.sh).
# Also, you need to restart the nodes after running this script in order for the
# update to be endorsed by the nodes.

if [ ! "$1" ]; then echo "update-6.sh: expects an <N> epoch argument"; exit; fi
...
COINS_IN_INPUT=((${INIT_SUPPLY} - 5000000))
FEE=1000000
...
TXID2=$(cardano-cli transaction txid --tx-file tx4.tx)
